...
switch ($action) {
    ...
    if($imageCheck){
        $message = '<p class="notice-red">An image by that name already exists.</p>';
    } elseif (empty($invId) || empty($imgName)) {
        $message = '<p class="notice-red">You must select a vehicle and image file for the vehicle.</p>';
    } else {
        ...
        if ($result) {
        $message = '<p class="notice-green">The upload succeeded.</p>';
        } else {
        $message = '<p class="notice-red">Sorry, the upload failed.</p>';
        }
    }
    ...
    if ($remove) {
    $message = "<p class='notice-green'>$filename was successfully deleted.</p>";
    } else {
    $message = "<p class='notice-red'>$filename was NOT deleted.</p>";
    }
    ...
    $imageDisplay = '<p class="notice-red">Sorry, no images could be found.</p>';
    }
